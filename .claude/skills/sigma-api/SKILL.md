...
The harness shows the skill's install path on the line "Base directory for this skill: ..." at the top of this file when loaded. Substitute that absolute path:

```sh
eval "$(<base-directory>/scripts/get-token.sh)"
```

For example, if the harness shows `Base directory for this skill: /Users/me/.claude/plugins/.../sigma-api`, run:

```sh
eval "$(/Users/me/.claude/plugins/.../sigma-api/scripts/get-token.sh)"
```

Do **not** rely on the `${CLAUDE_PLUGIN_ROOT}` environment variable — it isn't always set, and using it as a path component will silently expand to an empty string and produce a wrong absolute path like `/skills/sigma-api/scripts/get-token.sh`.
